...
class DOMInternals
{
	...
	static public function replace(child : Node, node : Node, parent : Node) : Node {
		...
		if (parent.nodeType != DOMConstants.DOCUMENT_NODE &&
			parent.nodeType != DOMConstants.DOCUMENT_FRAGMENT_NODE &&
			parent.nodeType != DOMConstants.ELEMENT_NODE) {
		...
		if (node.nodeType != DOMConstants.DOCUMENT_FRAGMENT_NODE &&
			node.nodeType != DOMConstants.DOCUMENT_TYPE_NODE &&
			node.nodeType != DOMConstants.ELEMENT_NODE &&
			node.nodeType != DOMConstants.TEXT_NODE &&
			node.nodeType != DOMConstants.PROCESSING_INSTRUCTION_NODE &&
			node.nodeType != DOMConstants.COMMENT_NODE) {
		...
		// throw a "HierarchyRequestError".
		if (node.nodeType == DOMConstants.TEXT_NODE && parent.nodeType == DOMConstants.DOCUMENT_NODE || 
			node.nodeType == DOMConstants.DOCUMENT_TYPE_NODE && parent.nodeType != DOMConstants.DOCUMENT_NODE) {
		...
		if (parent.nodeType != DOMConstants.DOCUMENT_NODE) {
			...
			if (node.nodeType == DOMConstants.DOCUMENT_FRAGMENT_NODE) {
				...
				if (node.childNodes.length > 1 || 
					node.childNodes.length == 1 && node.childNodes[0].nodeType == DOMConstants.TEXT_NODE) {
				...
				if (node.childNodes.length == 1) {
					...
					if (parent.childNodes.length == 1 && parent.childNodes[0] != child ||
						child.nextSibling != null && child.nextSibling.nodeType == DOMConstants.DOCUMENT_TYPE_NODE) {
			...
			if (node.nodeType == DOMConstants.DOCUMENT_TYPE_NODE) {
				// parent has a doctype child that is not child, or an element is preceding child.
				if (parent.childNodes.length == 1 && parent.childNodes[0] != child && 
					parent.childNodes[0].nodeType == DOMConstants.DOCUMENT_TYPE_NODE ||
					child.previousSibling != null && child.previousSibling.nodeType == DOMConstants.ELEMENT_NODE) {
		...
		var nodes = node.nodeType == DOMConstants.DOCUMENT_FRAGMENT_NODE ? node.childNodes : [node];
	...
	static public function replaceAll(node : Node, parent : Node) : Void {
		...
		*/
		...
		} else if (node.nodeType == DOMConstants.DOCUMENT_FRAGMENT_NODE) {
	...
	{
		if ( parent.nodeType != DOMConstants.DOCUMENT_NODE && parent.nodeType != DOMConstants.DOCUMENT_FRAGMENT_NODE && parent.nodeType != DOMConstants.ELEMENT_NODE  )
		{
			throw "HierarchyRequestError";
		}
		...
		if ( parent.nodeType == DOMConstants.DOCUMENT_NODE )
		{
			if ( node.nodeType != DOMConstants.DOCUMENT_FRAGMENT_NODE && node.nodeType != DOMConstants.DOCUMENT_TYPE_NODE && node.nodeType != DOMConstants.ELEMENT_NODE && 
					node.nodeType != DOMConstants.PROCESSING_INSTRUCTION_NODE && node.nodeType != DOMConstants.COMMENT_NODE )
			{
				throw "HierarchyRequestError";
			}
			if ( node.nodeType == DOMConstants.DOCUMENT_FRAGMENT_NODE )
			{
				if ( hasChild( node, DOMConstants.ELEMENT_NODE, 1 ) || hasChild( node, DOMConstants.TEXT_NODE ) )
				{
					throw "HierarchyRequestError";
				}
				if ( hasChild( node, DOMConstants.ELEMENT_NODE ) && 
					( hasChild( parent, DOMConstants.ELEMENT_NODE ) || child.nodeType == DOMConstants.DOCUMENT_TYPE_NODE || child != null && isFollowing( child, DOMConstants.DOCUMENT_TYPE_NODE ) ) )
				{
					throw "HierarchyRequestError";
				}
			}
			if ( node.nodeType == DOMConstants.ELEMENT_NODE &&
				( hasChild( parent, DOMConstants.ELEMENT_NODE ) || child != null && (child.nodeType == DOMConstants.DOCUMENT_TYPE_NODE || isFollowing( child, DOMConstants.DOCUMENT_TYPE_NODE)) ) )
			{
				throw "HierarchyRequestError";
			}
			if ( node.nodeType == DOMConstants.DOCUMENT_TYPE_NODE && 
				( hasChild( parent, DOMConstants.DOCUMENT_TYPE_NODE ) || child != null && isPreceding( child, DOMConstants.ELEMENT_NODE ) || child == null ) && 
					hasChild( parent, DOMConstants.ELEMENT_NODE ) )
			{
				throw "HierarchyRequestError";
			}
		}
		else if ( node.nodeType != DOMConstants.DOCUMENT_FRAGMENT_NODE && node.nodeType != DOMConstants.ELEMENT_NODE && node.nodeType != DOMConstants.TEXT_NODE && 
					node.nodeType != DOMConstants.PROCESSING_INSTRUCTION_NODE && node.nodeType != DOMConstants.COMMENT_NODE )
		{
			throw "HierarchyRequestError";
		}
	...
	{
		var count : Int = node.nodeType == DOMConstants.DOCUMENT_FRAGMENT_NODE ? node.childNodes.length : 1;
		...
		var nodes : NodeList = node.nodeType == DOMConstants.DOCUMENT_FRAGMENT_NODE ? node.childNodes.copy() : [node];
	...
	static public function firstAttr( node : Element, ?name : Null<String> = null ) : Null<Attr>
	{
		...
		{
			if ( name == null || node.attributes.item(i).name == name )
			{
