...
import cocktail.core.dom.Comment;
import cocktail.core.dom.Node;
import cocktail.core.dom.Element;
import cocktail.core.dom.ProcessingInstruction;
...
class HTMLSerializer
{
	static public function serialize( node : Node ) : String
	{
		...
		{
			if ( currentNode.nodeType == DOMConstants.ELEMENT_NODE )
			{
				...
				for ( i in 0...ce.attributes.length )
				{
					s.addChar( 0x20 );
					s.add( serializeAttributeName( ce.attributes.item(i).name ) );
					s.addChar( 0x3D );
					s.addChar( 0x22 );
					s.add( escape( ce.attributes.item(i).value ) );
					s.addChar( 0x22 );
				}
				s.addChar(0x3E);
				...
				{
					if ( ce.childNodes.length > 0 && ce.childNodes[0].nodeType == DOMConstants.TEXT_NODE && cast(ce.childNodes[0], Text).data.charCodeAt(0) == 0xA )
					{
						s.addChar( 0xA );
					}
				}
			...
			else if ( currentNode.nodeType == DOMConstants.TEXT_NODE )
			{
			...
			else if ( currentNode.nodeType == DOMConstants.COMMENT_NODE )
			{
			...
			else if ( currentNode.nodeType == DOMConstants.PROCESSING_INSTRUCTION_NODE )
			{
			...
			else if ( currentNode.nodeType == DOMConstants.DOCUMENT_TYPE_NODE )
			{
				s.addChar( 0x3C );
				s.addChar( 0x21 );
				s.addChar( 0x44 );
